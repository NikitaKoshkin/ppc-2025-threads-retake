...
void koshkin_n_convex_hull_binary_image_tbb::ConvexHullBinaryImage::FindPoints() {
  ...
  cv.reserve(static_cast<std::size_t>(std::min(1024, w * h)));  // hint, не обязателен

  // parallel iteration over rows; each thread pushes into concurrent_vector
  ...
  // Move into points_ vector (sequential)
...
bool koshkin_n_convex_hull_binary_image_tbb::ConvexHullBinaryImage::RunImpl() {
  ...
  // lexicographic sort and unique (parallel sort)
  ...
  // Find pivot: minimal y then minimal x (parallel_reduce)
  ...
  auto pivot_pair = tbb::parallel_reduce(
      ...
      [&](const std::pair<Pt, bool>& a, const std::pair<Pt, bool>& b) -> std::pair<Pt, bool> {
        ...
        if (pa.second < pb.second || (pa.second == pb.second && pa.first < pb.first)) return a;
        return b;
      });
  Pt pivot = pivot_pair.first;

  // Polar sort relative to pivot (parallel)
  tbb::parallel_sort(points_.begin(), points_.end(), [&](const Pt& a, const Pt& b) {
    if (a == b) return false;
